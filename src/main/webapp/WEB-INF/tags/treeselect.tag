...
<%@ attribute name="secondLabelIds" type="java.lang.String" required="false" description="第二输入框Ids（Name）"%>
<%@ attribute name="lastDataValueLabel" type="java.lang.String" required="false" description="只能选择最底层数据节点"%>
<%@ attribute name="expandAll" type="java.lang.Boolean" required="false" description="只能选择最底层数据节点"%>
...
<script type="text/javascript">
	$("#${id}Button").click(function(){
		...
		// 正常打开	
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&nodesLevel=${nodesLevel}&expandAll=${expandAll}&selectIds="+$("#${id}Id").val(), "选择${title}", 300, 420, {
			buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					var tree = h.find("iframe")[0].contentWindow.tree;//h.find("iframe").contents();
					var ids = [], names = [], nodes = [], pNames = [], ppNames = [];
					if ("${checked}" == "true"){
					...
					for(var i=0; i<nodes.length; i++) {
						if ("${lastDataValueLabel}"!=""&&typeof(nodes[i].pName)=='undefined'){
							top.$.jBox.tip("不能选择“非${lastDataValueLabel}”节点（"+nodes[i].name+"）请重新选择。");
							return false;
						}
					}
					...
					$("#${id}Name").val(names);
					var tpn = pNames.join(" ");
					var tpnArr = tpn.split(";");
					if("${secondLabelIds}"!=""){
						var sLabelIdArr = "${secondLabelIds}".split(",");
						for(var i = 0;i<sLabelIdArr.length;i++){
							$("#"+sLabelIdArr[i]).val(tpnArr[i]);
						}
					}
